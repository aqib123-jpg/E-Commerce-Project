...
export default function Biscuits() {
  ...
  useEffect(() => {
    fetchProducts('BISCUITSPRODUCTS');
  }, []);
  ...
  return (
    <>
    <div className='text-2xl font-bold font-sans text-center md:mt-12 mt-28'>Biscuits</div>
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mx-4 mt-8 mb-4 text-center">
        {products.slice().reverse().map(product => (
            <div key={product.ID} className="border rounded-lg p-4 shadow-2xl bg-white flex flex-col h-full">
                <img src={product.IMAGE_URL} alt={product.BISCUITS_PRODUCTS_NAME} className="w-full md:h-48 h-64 object-cover rounded-t-lg mb-4 hover:scale-105"/>
                <div className="flex flex-col flex-grow">
                    <div className="text-xl font-semibold mb-2">{product.BISCUITS_PRODUCTS_NAME}</div>
                </div>
                <div className="text-lg text-gray-500 mb-4">{product.PRICE}</div>
                <button className="bg-teal-600 text-white px-4 py-2 my-1 rounded-full hover:bg-teal-800 mt-auto"  onClick={() => handleButtonClick(product.BISCUITS_PRODUCTS_NAME,product.PRICE,product.IMAGE_URL)}>Details</button>
                {
                  (Number(localStorage.getItem('userId')) === 86) ?
                  <button className="bg-[#E86C4F] text-white px-4 py-2 rounded-full hover:bg-[#D6452A] mt-auto" onClick={() => confirmProductDelete(product.ID,'BISCUITSPRODUCTS',product.BISCUITS_PRODUCTS_NAME)}>Delete</button>
                  :
                  null
                }
            </div>
